...
void *func(void *cli)
{
    ...
    int n;
    memset(buff, 0, MAX);
    sprintf(buff, "[%s] %s Connected.", name, c->name);
    printf("%s\n", buff);
    ...
    while(1) {
        ...
        if (!n) {
            sprintf(buff, "[%s] %s has left", name, c->name);
            send_all(buff, c->id);
            printf("%s\n", buff);
            break;
        }
        // else if(!strncasecmp(buff, "hi", 2)) {
        //     sprintf(buff, "[%s] Co-PI %s is here!", name, c->name);
        //     send_all(buff, c->id);
        //     printf("%s\n", buff);
        // }
        else if (buff[0] == '@') {
...
int main()
{
    int sockfd, connfd, len, option=1, password, entered;
    char cred[40];
    ...
    while(1)
    {
        ...
        if (cnum == MAX_CLIENTS-1 || connfd < 0) {
            ...
            char cname[20];
            if(!read(connfd, cred, sizeof(cred))) {
                printf("[-]no password\n");
                close(connfd);
                continue;
            }
            sscanf(cred, "%s%d", cname, &entered);
            if(entered != password) {
                printf("[-]Permission Denied!\n");
                write(connfd, "Permission Denied!", 19);
                close(connfd);
                continue;
            }
            else {
                char buff[MAX];
                memset(buff, 0, MAX);
                sprintf(buff, "[%s] Welcome to project COBRA!\n", name);
                ...
                if(none) {
                    ...
                    write(connfd, "No other Co-PIs connected. Pl. wait.", 37);
                }


                printf("[+]New connection: %s [%d.%d.%d.%d:%d]\n", cname, cli.sin_addr.s_addr & 0xff,
                    (cli.sin_addr.s_addr & 0xff00) >> 8,
                    (cli.sin_addr.s_addr & 0xff0000) >> 16,
                    (cli.sin_addr.s_addr & 0xff000000) >> 24, ntohs(cli.sin_port));
                ...
                strcpy(clients[cnum]->name, cname);
